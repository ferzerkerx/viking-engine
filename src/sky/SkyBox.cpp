...
SkyBox::SkyBox(float alto, float largo, float ancho, unsigned int* caras){
	if (alto <= 0.0f){alto = 10.0f;}if (largo <= 0.0f){largo = 10.0f;} if (ancho <= 0.0f){ancho = 10.0f;}
	m_alto = alto /2.0f;
	m_largo = largo /2.0f;
	m_ancho = ancho/2.0f;
...
void SkyBox::render(vector3f poscam){
	...
	glPushMatrix();
		...
		glBegin(GL_QUADS);	
			glTexCoord2f(0.0f, 0.0f); glVertex3f(m_largo,-m_alto,-m_ancho);
			glTexCoord2f(1.0f, 0.0f); glVertex3f(-m_largo,-m_alto,-m_ancho);
			glTexCoord2f(1.0f, 1.0f); glVertex3f(-m_largo,m_alto,-m_ancho);
			glTexCoord2f(0.0f, 1.0f); glVertex3f(m_largo,m_alto,-m_ancho);
		glEnd();
