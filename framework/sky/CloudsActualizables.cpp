...
int CloudsActualizables::actualizaPorOctavo(){
	...
	if ((m_actual_octave < m_num_octaves) && (act_millis >= (m_update_text_milli/(m_num_octaves+1))*m_actual_octave)){
		Perlin::overlapOctave(m_map32, m_map,m_text_width,m_text_height,m_actual_octave,m_octaves[m_actual_octave].scale);
		m_actual_octave++;		
	}
...
int CloudsActualizables::actualizaPorCoordenada(){
	...
	if ((m_actual_octave < m_num_octaves) && (act_millis >= (m_update_text_milli/(m_num_octaves+1))*m_actual_octave)){
		...
		for (int y=0; y < m_text_height; y++){
				m_brightness = Perlin::interpolate(m_octaves[m_actual_octave].x*m_octaves[m_actual_octave].scale, y*m_octaves[m_actual_octave].scale , m_map32);
