...
#include "MD2Loader.h"
#include "..\loggers\HTMLEventLogger.h"
#include "..\textura.h"
...
void MD2Loader::importar(char* modelo, char* textura){
	...
	if (!m_file){
		LOG(LOG_COLOR_DK_RED , "No existe el archivo: %s", modelo);
		return;
	}
	struct md2_header m_header;

	fread(&m_header, 1, sizeof(struct md2_header), m_file);
	LOG(LOG_COLOR_DK_GREEN, "ident = %d",m_header.ident);
	LOG(LOG_COLOR_DK_GREEN, "version = %d",m_header.version);
	LOG(LOG_COLOR_DK_GREEN, "skinwidth = %d",m_header.skinwidth);
	LOG(LOG_COLOR_DK_GREEN, "skinheight = %d",m_header.skinheight);
	LOG(LOG_COLOR_DK_GREEN, "framesize = %d",m_header.framesize);
	LOG(LOG_COLOR_DK_GREEN, "num_skins = %d",m_header.num_skins);
	LOG(LOG_COLOR_DK_GREEN, "num_vertices = %d",m_header.num_vertices);
	LOG(LOG_COLOR_DK_GREEN, "num_st = %d",m_header.num_st);
	LOG(LOG_COLOR_DK_GREEN, "num_tris = %d",m_header.num_tris);
	LOG(LOG_COLOR_DK_GREEN, "num_glcmds = %d",m_header.num_glcmds);
	LOG(LOG_COLOR_DK_GREEN, "num_frames = %d",m_header.num_frames);
	LOG(LOG_COLOR_DK_GREEN, "offset_skins = %d",m_header.offset_skins);
	LOG(LOG_COLOR_DK_GREEN, "offset_st = %d",m_header.offset_st);
	LOG(LOG_COLOR_DK_GREEN, "offset_tris = %d",m_header.offset_tris);
	LOG(LOG_COLOR_DK_GREEN, "offset_frames = %d",m_header.offset_frames);
	LOG(LOG_COLOR_DK_GREEN, "offset_glcmds = %d",m_header.offset_glcmds);
	LOG(LOG_COLOR_DK_GREEN, "offset_end = %d",m_header.offset_end);

	if (m_header.ident != IDENT || m_header.version != VERSION){
		LOG(LOG_COLOR_DK_RED | LOG_BOLD, "Error al cargar \"%s\" version de MD2 incorrecta", modelo);
		return;
	}
...
void MD2Loader::leeMD2Data(md2_header  & m_header){
	...
	for(i=0; i<m_header.num_frames; i++){
		...
		for (j=0; j < m_header.num_vertices; j++){
			...
			pVertices[j].vertex[2] = (pFrame->aliasVertices[j].vertex[1] * pFrame->scale[1] + pFrame->translate[1]) * -1;
...
void MD2Loader::agregaAnimacion(int start, int end, char * name){
	...
	LOG(LOG_COLOR_DK_RED | LOG_ITALICS, "start= %d end= %d nombre = %s",anim.frame_inicial, anim.frame_final, anim.nombre);
}
